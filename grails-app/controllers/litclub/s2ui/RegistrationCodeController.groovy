 ...
 */
class RegistrationCodeController extends litclub.s2ui.AbstractS2UiController {
	...
	def registrationCodeSearch = {
		...
		if (params.domain) {
			hql.append " AND LOWER(rc.domain) LIKE :domain"
			queryParams.username = params.domain.toLowerCase() + '%'
		}
	...
	def ajaxRegistrationCodeSearch = {
		...
		if (params.term?.length() > 2) {
			...
			def results = RegistrationCode.executeQuery(
					...
					"WHERE LOWER(rc.domain) LIKE :name " +
					"ORDER BY rc.domain ",
